...
function Draggable({ sections, handleDragEnd, handleOpenCustomModal, handleUpdateSection, removeItem, handleClickedVideo, handleClickedUpdateRows, selectedVideoId, setRowNumber, handleUpdateVideo, handleClickWatchingVideo, setVideoDetail, inputRef, buttonRef, clickDownloadFile, handleRemoveFile, isEnable, setSectionModel, setIsEnable, setVideoId }) {
    return (
                          ...
                          {section.videos.map((video, videoIndex) => (
                            <div key={video.videoId}>
                              <div style={{ marginTop: '10px', padding: '10px', border: '1px solid lightgray', borderRadius: '4px', backgroundColor: 'lightgray' }}>
                                <input
                                  ref={inputRef}
                                  disabled={selectedVideoId !== video.videoId}
                                  onChange={(e) => setRowNumber(e.target.value)}
                                  defaultValue={video.rowNumberForSection}
                                />
                                <button
                                  id={video.videoId}
                                  className='btn btn-warning'
                                  onClick={() => handleClickedVideo(video.videoId)}
                                >
                                ...
                                {handleClickedUpdateRows && selectedVideoId === video.videoId && (
                                  <button className='btn btn-success' onClick={() => handleUpdateVideo(video.videoId)}>Save Changes</button>
                                )}
                                ...
                                <div style={{ position: "relative" }}>
                                  {video.materials.length > 0 ? (
                                    ...
                                    <button
                                      ...
                                      onClick={() => { handleOpenCustomModal("NewMaterial"); setVideoId(video?.videoId) }}
                                    >
                                      Add Material
                                    </button>
                                  )}
