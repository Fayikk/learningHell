...
// Course kartı bileşeni
const CourseCard = React.memo(({ course }) => {
  const rating = useMemo(() => calculateAverageRating(course.ratings), [course.ratings]);
  ...
  return (
    <>
      <Link
            ...
            >
    <div className="wpo-popular-single" tabIndex="0" role="article">
      <div className="wpo-popular-item">
        <div className="wpo-popular-img">
          {!imageLoaded && <div className="skeleton-loading" style={{height: "200px"}} />}
          <img 
            ...
            loading="lazy"
          />          <div 
            className={`thumb ${course.coursePrice === "0" || course.coursePrice === 0 ? 'free' : ''}`} 
            role="text" 
            style={{ display: imageLoaded ? 'block' : 'none' }}
            aria-label={`Course price: ${course.coursePrice === "0" ? "FREE" : `${MatchLocationToCurrency()}${course.coursePrice}`}`}
          >
        ...
        <div className="wpo-popular-content">
          <div className="wpo-popular-text-top">
            {/* <ul>
              <li>
                <img src={course.author} alt={`${course.authortitle} profile`} />
              </li>
              <li>
                <Link
                  onClick={ClickHandler}
                  to={`/course-single/${course.courseId}`}
                  aria-label={`View instructor ${course.authortitle}'s profile`}
                >
                  {course.authortitle}
                </Link>
              </li>
            ...
            <ul className="rating-stars">
              <li>
                <i className="fi flaticon-star" aria-hidden="true"></i>
              </li>
              <li aria-label={`Course rating: ${rating} out of 5`}>({rating})</li>
            </ul>
          </div>
          <h2>
          ...
          <div className="wpo-popular-text-bottom">
            <ul>
              <li>
                <i className="fi flaticon-reading-book" aria-hidden="true"></i>
                <span aria-label={`${course.student} enrolled students`}>
                  {course.student} Students
                </span>
              </li>
              <li>
                <i className="fi flaticon-agenda" aria-hidden="true"></i>
                <span aria-label={`${course.lesson} total lessons`}>
              ...
              <div className="row">
                {[1,2,3].map((_, index) => (
                  <div className="col col-lg-4 col-md-6 col-12" key={index}>
                    <div className="wpo-popular-single skeleton-loading" style={{height: "400px"}} />
                  </div>
                ))}
              </div>
            ) : (
